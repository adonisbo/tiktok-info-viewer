...
import axios from 'axios'; // 使用 ES Module import
...
export const fetchTikTokUserProfile = async (tiktokUserId, jinaApiKey) => {
  ...
  try {
    ...
    // 为了调试，可以取消注释下一行来查看完整的Jina响应体结构
    // console.log('[jinaService] Full Jina API Response Data:', JSON.stringify(response.data, null, 2));


    // --- 修改点在这里 ---
    // 检查 response.data.data.content 是否存在
    ...
    return response.data.data.content; // 返回 response.data.data.content
    // --- 修改结束 ---
...
// 函数声明前已使用 export, 所以不需要下面这行
// export { fetchTikTokUserProfile };